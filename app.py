...
# 画面の基本設定 & 機種スペック定義
# ==========================================
st.set_page_config(page_title="ジャグラー 予測AI", layout="wide")

# 各機種ごとの設定（URLキー、設定3と4の中間RB確率、角台リスト）
MACHINE_CONFIG = {
    "マイジャグラー5": {
        "url_secret_key": "spreadsheet_url_myjug",
        "target_rb_prob": 313.0,    # 設定3(1/336)と4(1/290)の中間
        "corner_list": [521, 540, 541, 560]
    },
    "ゴーゴージャグラー3": {
        "url_secret_key": "spreadsheet_url_gogo",
        "target_rb_prob": 287.0,    # 設定3(1/306)と4(1/268)の中間
        "corner_list": [601, 620, 621, 640] # ※実際のホールの角台番号に書き換えてください
    },
    "ハッピージャグラーV": {
        "url_secret_key": "spreadsheet_url_happy",
        "target_rb_prob": 318.0,    # 設定3(1/336)と4(1/300)の中間
        "corner_list": [701, 720, 721, 740] # ※実際のホールの角台番号に書き換えてください
    }
}
...
# サイドバー（機種選択 & 絞り込み設定）
# ==========================================
st.sidebar.header("🎰 機種選択")
selected_machine = st.sidebar.selectbox(
    "分析する機種を選択",
    list(MACHINE_CONFIG.keys())
)
config = MACHINE_CONFIG[selected_machine]
...
st.title(f"🎰 {selected_machine} 狙い目予測AI & 傾向分析")
...
def load_data(url_secret_key):
    cache_buster = int(time.time())
    try:
        base_url = st.secrets[url_secret_key]
    except KeyError:
        st.error(f"❌ セキュリティ用の設定 ({url_secret_key}) が見つかりません。")
        st.stop()
...
with st.spinner(f'{selected_machine}のデータを読み込み中...'):
    df = load_data(config["url_secret_key"])
...
# 機種ごとの角台リストを適用
corner_list = config["corner_list"]
...
# 共通の危険台除外ルール
# ==========================================
# 機種ごとの「設定3〜4の境界RB確率」をボーダーとして適用
target_prob = config["target_rb_prob"]
...
exclude_condition = (
    ...
    ((latest_df['RB確率'] > 0) & (latest_df['RB確率'] <= target_prob)) |
    ((latest_df['1日前のRB確率'] > 0) & (latest_df['1日前のRB確率'] <= target_prob))
)
...
# 過去の高設定台（正解データ）を抽出（機種ごとのボーダーを適用）
high_setting_days = df[(df['RB確率'] > 0) & (df['RB確率'] <= target_prob) & (df['差枚'] >= 1000)].copy()
